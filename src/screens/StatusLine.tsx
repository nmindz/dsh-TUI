...
const selectionBadge = formatSelectionBadge(channel.selection)
  ...
  // Live IDE-selection badge: it tracks the user's in-editor gesture, the
  // freshest signal in the footer (T-FIX-02). Absent without an IDE — no
  // placeholder, matching the official Claude Code footer. Like `jobs` it is
  // transient state rather than chrome, so it is never layout-addressable and
  // always leads the right group.
  const idePart: FieldPart[] = selectionBadge === undefined
    ? []
    : [{
        key: 'ideSelection',
        node: <Text color="ide" wrap="truncate">{selectionBadge}</Text>,
      }]
  ...
  if (layoutTokens !== undefined) {
    ...
    rightFields = [...idePart, ...resolveTokens(rightTokens)]
  ...
  const detail = buildHoverDetail(hover, channel, usage, contextUsed, columns, barColors, pluginSegments)
...
function buildHoverDetail(
  ...
  // Plugin segments answer a hover only when they supplied a detail.
