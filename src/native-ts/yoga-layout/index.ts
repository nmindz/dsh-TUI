...
export class Node {
   ...
   * (aW,aH,wM,hM,oW,oH,fW,fH) and [i*3, i*3+3) in _cOut
   * (w,h,measuredWidth).
   */
...
function cacheWrite(
  ...
  if (!node._cIn) {
    ...
    node._cOut = new Float64Array(CACHE_SLOT_CAPACITY * 3)
  }
...
function layoutNode(
  ...
  if (!node.isDirty_ || sameGen) {
    ...
    // state belongs to. Rationale in docs/rendering-performance.md.
    if (
      node._cN > 0 &&
      (!performLayout ||
