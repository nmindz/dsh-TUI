...
const dict = {
  ...
  'preset-unavailable': { zh: 'Preset 不可用——当前组合未挂载 agent preset 注册表', en: 'Preset unavailable — the agent preset registry is not mounted' },
  ...
  'preset-roster-unmounted': { zh: '当前组合未挂载 agent preset 注册表（preset 不可用）', en: 'The agent preset registry is not mounted (presets unavailable)' },
  ...
  'reload-footer': { zh: '提示：profile 的 cordis.patch.yml 由 watcher 自动热重载；cordis.yml 根配置改动需 /restart', en: 'Note: the profile cordis.patch.yml hot-reloads via its watcher; cordis.yml root config changes need /restart' },
